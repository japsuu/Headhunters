...
public class AddonTheForestLike : AddonBehaviour
{
    ...
    private void Awake()
    {
        ...
        Piece.ChangeState(StateType.Queue);
    }
    ...
    IEnumerator DelayedStart()
    {
        ...
        if (Preview == null)
        {
            GameObject prefab = BuildManager.Instance.GetPieceById(Piece.Id).gameObject;
            
            Preview = Instantiate(prefab, transform.position, transform.rotation);
    ...
    /// </summary>
    //WARN: Set to this to server method!!!
    public void Upgrade(string materialTag)
    {
        Piece.SkinIndex++;

        gameObject.ChangeAllMaterialsInChildren(Piece.Renderers.ToArray(), Piece.InitialRenderers);

        Elements.FirstOrDefault(x => !x.gameObject.activeSelf && x.CompareTag(materialTag))?.gameObject.SetActive(true);

        //WARN: At this point, we would remove the Carriable from the player on the server side
